...
export async function gitCliTool(config: AppConfig, workspace: Workspace, cmd: string[], cwdPath = '.', timeoutMs = 60000, maxOutputChars = 20000) {
  ...
  return withGitAuth(workspace, async (env) => {
    ...
    const lfsInitialized = await initializeLfsForWorktreeAdd(executable, cmd, cwd.absolute, timeout, env, environmentMode);
    const result = await runCommand(executable, cmd.map(String), cwd.absolute, timeout, env, environmentMode);
    const output = truncateText(redactGitOutputForDisplay(`${result.stdout}${result.stderr}`, sanitize), Math.min(Math.max(1, maxOutputChars), 50000));
    return ok('git command finished', {
      command: ['git', ...cmd].map((value) => redactGitOutputForDisplay(value, sanitize)), cwd: cwd.displayPath,
...
export async function gitPushCurrentBranch(config: AppConfig, workspace: Workspace, repoPath = '.', remote = 'origin', branch?: string) {
  ...
  return withGitAuth(workspace, async (env) => {
    ...
    return ok('git push finished', gitPushResult(workspace, context.cwd, remote, context.remoteUrl, context.branch, context.sha, result, sanitizeResultsEnabled(config) || redactSecretValuesEnabled(config)));
  });
}
...
async function gitLfsPublish(config: AppConfig, workspace: Workspace, context: Awaited<ReturnType<typeof gitPublishContext>>, env: Record<string, string>, forceWithLeaseSha?: string, autoSelected = false) {
  ...
  for (const command of [['lfs', 'version'], ['lfs', 'install', '--local'], ['lfs', 'fsck', '--pointers', 'HEAD'], ['lfs', 'push', context.remote, context.branch]]) {
    const step = await runGitPublishStep(context.executable, command, context.cwd, config.security.max_exec_ms, env, workspaceChildEnvironmentMode(config, workspace), sanitizeResultsEnabled(config) || redactSecretValuesEnabled(config));
    steps.push(step);
  ...
  steps.push(await runGitPublishStep(context.executable, push, context.cwd, config.security.max_exec_ms, env, workspaceChildEnvironmentMode(config, workspace), sanitizeResultsEnabled(config) || redactSecretValuesEnabled(config)));
...
async function initializeLfsForWorktreeAdd(executable: string, cmd: string[], cwd: string, timeout: number, env: Record<string, string>, environmentMode: 'full' | 'minimal'): Promise<boolean> {
...
function gitPushResult(workspace: Workspace, cwd: string, remote: string, remoteUrlRaw: string, branch: string, sha: string, result: { code: number | null; stdout: string; stderr: string; timed_out: boolean }, sanitize: boolean) {
  const output = truncateText(redactGitOutputForDisplay(result.stdout + result.stderr, sanitize), 50000);
  ...
  return {
    ...
    remote,
    remote_url: sanitizeGitRemoteForDisplay(remoteUrlRaw, sanitize),
    branch,
    sha,
    publish_mode: 'git',
...
async function runGitPublishStep(executable: string, args: string[], cwd: string, timeout: number, env: Record<string, string>, environmentMode: 'full' | 'minimal', sanitize: boolean) {
...
export function sanitizeGitRemoteForDisplay(url: string, enabled = false): string {
  return enabled ? url.replace(/(https?:\/\/)([^/@:]+)(:[^/@]+)?@/g, '$1') : url;
}

export function redactGitOutputForDisplay(text: string, enabled = false): string {
...
function safeGitMessage(text: string, sanitize = false): string {
  return limit(redactGitOutputForDisplay(text || 'unknown git failure', sanitize));
}
