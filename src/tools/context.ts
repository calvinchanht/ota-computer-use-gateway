...
function bootstrapNextActions(workspace: Workspace) {
  return [
    ...
    workspace.allow_read ? (workspace.api_sets?.machine_admin && workspace.filesystem?.machine_admin_host_scope ? 'Confirm file access by calling list_dir on "." for workspace-relative context and, when needed, stat_path/read_file on explicit absolute host paths inside host_root. Denied secret paths remain protected; never paste raw secret contents unless the operator explicitly asks.' : 'Confirm scoped file access by calling list_dir on ".", and when useful tree/stat_path/read_file anywhere inside the workspace. Do not treat secrets/ or credential-named files as unavailable when gateway policy grants workspace access. Avoid pasting raw secret contents into chat unless the operator explicitly asks.') : 'Read access is disabled by policy; say so explicitly if asked for files.',
