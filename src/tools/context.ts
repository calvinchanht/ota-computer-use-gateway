...
function bootstrapNextActions(workspace: Workspace) {
  return [
    ...
    'Use browser_cdp_call/browser_cdp_batch or browser_cdp_browser_call/browser_cdp_browser_batch for direct CDP automation when browser control is enabled. For any new page, use Target.createTarget; the gateway forces it into a separate Chrome window so the anchored Threaddex/ChatGPT tab is never displaced inside its window. Do not use window.open for page creation. Do not look for wrapper browser action tools; the gateway intentionally proxies CDP instead.',
