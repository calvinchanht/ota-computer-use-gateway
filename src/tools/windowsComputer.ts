...
async function psJson(script: string): Promise<unknown> {
  ...
  const { stdout, stderr } = await execFileAsync('powershell.exe', ['-NoProfile', '-Sta', '-ExecutionPolicy', 'Bypass', '-EncodedCommand', encoded], { timeout: 30000, maxBuffer: MAX_POWERSHELL_BUFFER, windowsHide: true });
...
function uiaSetValueScript(hwnd: number, selector: Required<WindowsUiaSelector>, value: string) {
  return `${formsAssemblies()}; ${uiaAssemblies()}; ${win32WindowTypes()}; ${uiaSelectorFn()}; $e=findUiaElement ${int(hwnd)} ${q(selector.automation_id)} ${q(selector.name)} ${q(selector.control_type)}; $e.SetFocus();$focused=[System.Windows.Automation.AutomationElement]::FocusedElement;if($null -eq $focused){throw 'UI Automation descendant focus verification failed'};if(-not [System.Windows.Automation.Automation]::Compare($e,$focused)){throw 'UI Automation descendant focus verification failed'}; $method=''; try{$p=$e.GetCurrentPattern([System.Windows.Automation.ValuePattern]::Pattern);if($p.Current.IsReadOnly){throw 'UI Automation ValuePattern is read-only'};$p.SetValue(${q(value)});$method='ValuePattern'}catch{if($_.Exception.Message -like '*read-only*'){throw}}; if(-not $method){try{$p=$e.GetCurrentPattern([System.Windows.Automation.LegacyIAccessiblePattern]::Pattern);$p.SetValue(${q(value)});$method='LegacyIAccessiblePattern'}catch{}}; if(-not $method -and $e.Current.NativeWindowHandle -ne 0){if([Win32Windows]::SetControlText([IntPtr]$e.Current.NativeWindowHandle,${q(value)})){$method='WM_SETTEXT'}}; if(-not $method){$focus=[Win32Windows]::Focus([IntPtr]${int(hwnd)});if(-not $focus.focused){throw ('failed to focus hwnd ${int(hwnd)}; foreground hwnd is '+$focus.foreground_hwnd)};$e.SetFocus();$focused=[System.Windows.Automation.AutomationElement]::FocusedElement;if($null -eq $focused){throw 'UI Automation descendant focus verification failed'};if(-not [System.Windows.Automation.Automation]::Compare($e,$focused)){throw 'UI Automation descendant focus verification failed'};Start-Sleep -Milliseconds 100;[System.Windows.Forms.SendKeys]::SendWait('^{A}');Start-Sleep -Milliseconds 100;[System.Windows.Forms.SendKeys]::SendWait(${q(sendKeysEscape(value))});$method='SetFocus+SendKeys'}; @{ hwnd=${int(hwnd)}; automation_id=$e.Current.AutomationId; name=$e.Current.Name; control_type=($e.Current.ControlType.ProgrammaticName -replace '^ControlType\\.'); set=$true; method=$method; value_chars=${value.length}; focus_method='AutomationElement.SetFocus'; focus_verified_before_mutation=$true; focus_verification='Automation.Compare' } | ConvertTo-Json -Depth 5`;
}
