...
type LoadedChange = PatchChange & { absolute: string; before_hash: string; after_hash: string };
...
async function loadChange(config: AppConfig, workspace: Workspace, change: PatchChange): Promise<LoadedChange> {
  ...
  return { ...change, path: resolved.relative, absolute: resolved.absolute, before_hash: sha256(current), after_hash: sha256(next) };
}
