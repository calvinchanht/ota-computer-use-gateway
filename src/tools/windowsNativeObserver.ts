...
public static class OtaBoundedNativeObserver
{
    ...
    public static object Observe(uint pid, long hwndValue, int timeoutMs, int eventCap)
    {
        ...
        try
        {
            ...
            if (RecordCount() >= eventCap) endedReason = "event_cap";
        }
        finally
        {
    ...
    private static void ValidateTarget(uint pid, IntPtr hwnd)
    {
        ...
        if (hwndPid != pid) throw new InvalidOperationException("native observer target PID/HWND mismatch");
        ...
        string targetSid = ProcessUserSid(pid);
    ...
    private static string ProcessUserSid(uint pid)
    {
        ...
        if (process == IntPtr.Zero) throw new Win32Exception(Marshal.GetLastWin32Error(), "cannot query native observer target process");
        try
        {
            IntPtr token;
    ...
    {
        if (nCode >= 0 && RecordCount() < EventCap)
        {
    ...
    {
        if (!AllowedWinEvents.Contains(eventType) || hwnd == IntPtr.Zero || RecordCount() >= EventCap) return;
    ...
    private static void AddRecord(Dictionary<string, object> record)
    {
        lock (Gate)
        {
            if (Records.Count >= EventCap) return;
