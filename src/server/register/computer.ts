...
function registerWindowsTools({ server, workspaces }: RegisterContext): void {
  ...
  server.registerTool('windows_window_screenshot', { title: 'Windows window screenshot', description: 'Capture one top-level hwnd without exposing competing image URLs. Uses the same OTA-to-Threaddex visual-followup contract as monitor screenshots.', inputSchema: { workspace_id: z.string(), hwnd: z.number().int().finite(), visual_followup: visualFollowupSchema, job_id: z.string().optional(), threaddex_job_id: z.string().optional() }, outputSchema: TOOL_RESULT_OUTPUT_SCHEMA, annotations: RUN_LOCAL }, async (args) => runWorkspaceTool(workspaces, args.workspace_id, 'windows_window_screenshot', (workspace) => windowsWindowScreenshot(workspace, args.hwnd, args)));
  server.registerTool('windows_window_screenshot_sequence', { title: 'Windows window screenshot sequence', description: 'Capture 2-8 ordered frames from one top-level hwnd and submit all previews to Threaddex together as one visual follow-up. The requested frame span is capped at 5 seconds.', inputSchema: { workspace_id: z.string(), hwnd: z.number().int().finite(), interval_ms: z.number().int().min(50).default(250), count: z.number().int().min(2).max(8).default(8), visual_followup: visualFollowupSchema, job_id: z.string().optional(), threaddex_job_id: z.string().optional() }, outputSchema: TOOL_RESULT_OUTPUT_SCHEMA, annotations: RUN_LOCAL }, async (args) => runWorkspaceTool(workspaces, args.workspace_id, 'windows_window_screenshot_sequence', (workspace) => windowsWindowScreenshotSequence(workspace, args.hwnd, args.interval_ms, args.count, args)));
