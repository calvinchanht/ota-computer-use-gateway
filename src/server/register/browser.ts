...
function registerListBrowserProfiles({ server, workspaces }: RegisterContext): void {
  server.registerTool('list_browser_profiles', { title: 'List browser profiles', description: 'List configured headed Chrome/CDP browser profiles for a workspace.', inputSchema: { workspace_id: z.string() }, outputSchema: TOOL_RESULT_OUTPUT_SCHEMA, annotations: READ_ONLY }, async (args) => runWorkspaceTool(workspaces, args.workspace_id, 'list_browser_profiles', listBrowserProfiles));
}

function registerBrowserStatus({ server, workspaces }: RegisterContext): void {
  server.registerTool('browser_status', { title: 'Browser status', description: 'Return selected headed Chrome/CDP profile status metadata.', inputSchema: profileSchema(), outputSchema: TOOL_RESULT_OUTPUT_SCHEMA, annotations: READ_ONLY }, async (args) => runWorkspaceTool(workspaces, args.workspace_id, 'browser_status', (workspace) => browserStatus(workspace, args.profile_label)));
}
...
function registerBrowserTabInfo({ server, workspaces }: RegisterContext): void {
  server.registerTool('browser_tab_info', { title: 'Browser tab info', description: 'Return metadata for one Chrome target/tab by id.', inputSchema: targetInfoSchema(), outputSchema: TOOL_RESULT_OUTPUT_SCHEMA, annotations: READ_ONLY }, async (args) => runWorkspaceTool(workspaces, args.workspace_id, 'browser_tab_info', (workspace) => browserTabInfo(workspace, args.target_id, args.profile_label)));
}

function registerBrowserTabScreenshot({ server, workspaces }: RegisterContext): void {
  server.registerTool('browser_tab_screenshot', { title: 'Browser tab screenshot', description: 'Capture a screenshot from one Chrome target/tab through CDP.', inputSchema: screenshotSchema(), outputSchema: TOOL_RESULT_OUTPUT_SCHEMA, annotations: READ_ONLY }, async (args) => runWorkspaceTool(workspaces, args.workspace_id, 'browser_tab_screenshot', (workspace) => browserTabScreenshot(workspace, args.target_id, args.profile_label, args.format)));
}

function registerBrowserTabSnapshot({ server, workspaces }: RegisterContext): void {
  server.registerTool('browser_tab_snapshot', { title: 'Browser tab snapshot', description: 'Capture a bounded DOM snapshot from one Chrome target/tab through CDP.', inputSchema: targetInfoSchema(), outputSchema: TOOL_RESULT_OUTPUT_SCHEMA, annotations: READ_ONLY }, async (args) => runWorkspaceTool(workspaces, args.workspace_id, 'browser_tab_snapshot', (workspace) => browserTabSnapshot(workspace, args.target_id, args.profile_label)));
}

function registerOpenBrowserTab({ server, workspaces }: RegisterContext): void {
  server.registerTool('open_browser_tab', { title: 'Open browser tab', description: 'Open a URL in a new headed Chrome tab through CDP. Optionally assign a stable tab_key that can be used anywhere target_id is accepted.', inputSchema: { ...profileSchema(), url: z.string(), tab_key: tabKeySchema().optional(), observe_after: observeAfterSchema().optional() }, outputSchema: TOOL_RESULT_OUTPUT_SCHEMA, annotations: RUN_LOCAL }, async (args) => runWorkspaceTool(workspaces, args.workspace_id, 'open_browser_tab', (workspace) => openBrowserTab(workspace, args.url, args.profile_label, args.observe_after, args.tab_key)));
}

function registerNavigateBrowserTab({ server, workspaces }: RegisterContext): void {
  server.registerTool('navigate_browser_tab', { title: 'Navigate browser tab', description: 'Navigate an existing Chrome target/tab to a URL through CDP.', inputSchema: { ...targetActionSchema(), url: z.string() }, outputSchema: TOOL_RESULT_OUTPUT_SCHEMA, annotations: RUN_LOCAL }, async (args) => runWorkspaceTool(workspaces, args.workspace_id, 'navigate_browser_tab', (workspace) => navigateBrowserTab(workspace, args.target_id, args.url, args.profile_label, args.observe_after)));
}

function registerClickBrowserTab({ server, workspaces }: RegisterContext): void {
  server.registerTool('click_browser_tab', { title: 'Click browser tab', description: 'Click viewport coordinates in an existing Chrome target/tab through CDP.', inputSchema: { ...targetActionSchema(), x: z.number(), y: z.number() }, outputSchema: TOOL_RESULT_OUTPUT_SCHEMA, annotations: RUN_LOCAL }, async (args) => runWorkspaceTool(workspaces, args.workspace_id, 'click_browser_tab', (workspace) => clickBrowserTab(workspace, args.target_id, args.x, args.y, args.profile_label, args.observe_after)));
}

function registerTypeBrowserTab({ server, workspaces }: RegisterContext): void {
  server.registerTool('type_browser_tab', { title: 'Type into browser tab', description: 'Insert text into the focused element of an existing Chrome target/tab through CDP.', inputSchema: { ...targetActionSchema(), text: z.string().max(10000) }, outputSchema: TOOL_RESULT_OUTPUT_SCHEMA, annotations: RUN_LOCAL }, async (args) => runWorkspaceTool(workspaces, args.workspace_id, 'type_browser_tab', (workspace) => typeBrowserTab(workspace, args.target_id, args.text, args.profile_label, args.observe_after)));
}

function registerFillBrowserTabField({ server, workspaces }: RegisterContext): void {
  server.registerTool('fill_browser_tab_field', { title: 'Fill browser tab field', description: 'Set the value of an input or textarea selected by CSS selector through scoped CDP Runtime.evaluate.', inputSchema: { ...targetActionSchema(), selector: z.string().min(1).max(1000), value: z.string().max(10000) }, outputSchema: TOOL_RESULT_OUTPUT_SCHEMA, annotations: RUN_LOCAL }, async (args) => runWorkspaceTool(workspaces, args.workspace_id, 'fill_browser_tab_field', (workspace) => fillBrowserTabField(workspace, args.target_id, args.selector, args.value, args.profile_label, args.observe_after)));
}
