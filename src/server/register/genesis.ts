...
export function registerGenesisTools({ server, config }: RegisterContext): void {
  ...
  }, async () => safe(genesisBootstrap(sanitize)));
  ...
  server.registerTool('estate_overview', {
    ...
    inputSchema: {}, outputSchema: TOOL_RESULT_OUTPUT_SCHEMA, annotations: READ_ONLY
  }, async () => safe(genesisEstateOverview(sanitize)));
  ...
  }, async ({ agent }) => safe(genesisAgentDeepDive(agent, sanitize)));
  ...
  }, async ({ host }) => safe(genesisHostDeepDive(host, sanitize)));
  ...
  }, async ({ scope, target }) => safe(genesisSafeDiagnostic(scope, target, sanitize)));
}
