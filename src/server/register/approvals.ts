...
export function registerApprovalTools({ server, workspaces }: RegisterContext): void {
  server.registerTool('approval_status', { title: 'Approval status', description: 'List local workspace approvals.', inputSchema: { workspace_id: z.string() }, annotations: READ_ONLY }, async (args) => runWorkspaceTool(workspaces, args.workspace_id, 'approval_status', approvalStatus));
