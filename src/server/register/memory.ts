...
import { contextSnapshot, recordProgress } from '../../tools/context.js';
...
function registerContextSnapshot({ server, workspaces }: RegisterContext): void {
  server.registerTool('get_context_snapshot', {
    title: 'Get context snapshot',
    description: 'Return workspace identity, project instructions, continuity files, and recent memory tail.',
    inputSchema: { workspace_id: z.string() },
    annotations: READ_ONLY
  }, async (args) => runWorkspaceTool(workspaces, args.workspace_id, 'get_context_snapshot', contextSnapshot));
}

function registerProgressRecorder({ server, workspaces }: RegisterContext): void {
  server.registerTool('record_progress', {
    title: 'Record progress',
    description: 'Append a progress or handoff note to workspace continuity.',
    inputSchema: { workspace_id: z.string(), title: z.string(), body: z.string(), handoff: z.boolean().default(false) },
    annotations: WRITE_FILE
  }, async (args) => runWorkspaceTool(
    workspaces, args.workspace_id, 'record_progress',
    (workspace) => recordProgress(workspace, args.title, args.body, args.handoff)
  ));
}
