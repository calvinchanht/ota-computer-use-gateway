...
Mac mouse controls are split by coordinate space. `computer_screen_click`, `computer_screen_mouse_move`, `computer_screen_drag`, and `computer_screen_scroll` accept global screen coordinates and infer/translate the target window where native Cua requires a pid. `computer_window_click`, `computer_window_mouse_move`, `computer_window_drag`, and `computer_window_scroll` operate in a specific app/window/process context and require a current `pid` from `list_windows` or `get_window_state` where applicable. `button='right'` routes to native `right_click`; `click_count=2` routes to native `double_click`. Mouse move uses Cua's visible agent cursor overlay (`move_cursor`) rather than an app-delivered hover event. `cua_driver_call` calls one allowed raw Cua Driver method with native Cua Driver params. Read-only methods (`check_permissions`, `list_windows`, `get_screen_size`, `get_window_state`, `get_accessibility_tree`, `get_agent_cursor_state`, `get_cursor_position`, `screenshot`) require screen policy. Local input/control methods (`click`, `double_click`, `right_click`, `drag`, `scroll`, `move_cursor`, `hotkey`, `press_key`, `set_value`, `type_text`, `type_text_chars`, `zoom`) require mouse/keyboard policy.
