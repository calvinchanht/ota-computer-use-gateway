...
await expectText('get_agent_bootstrap', { workspace_id: workspaceId }, 'provider_acceptance', sessionId);
await expectText('get_agent_bootstrap', { workspace_id: workspaceId }, 'mickey-pickup', sessionId);
...
console.log(`mickey acceptance smoke ok (${writeProof ? 'with checkpoint' : 'read-only'})`);

async function checkpointAcceptance(sessionId) {
  const stamp = new Date().toISOString();
  await call('checkpoint_thread', {
    workspace_id: workspaceId,
    title: 'Mickey acceptance smoke',
    summary: `Mickey acceptance smoke passed at ${stamp}.`,
    next_steps: ['Continue Catalyst anchoring after Mickey proof is accepted.']
  }, sessionId);
  await expectText('get_agent_bootstrap', { workspace_id: workspaceId }, `Mickey acceptance smoke passed at ${stamp}`, sessionId);
}
