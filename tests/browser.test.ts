...
describe('browser profile tools', () => {
  ...
  it('fills input fields through scoped runtime evaluation', async () => {
    mockFetch([{ id: '1', type: 'page', webSocketDebuggerUrl: 'ws://cdp/1' }]);
    const sends = mockWebSocket({ result: { value: { ok: true, tag: 'input' } } });
    const data = (await fillBrowserTabField(controlWorkspace(), '1', 'input[name="email"]', 'hello@example.com')).data as any;
    expect(data.selector).toBe('input[name="email"]');
    expect(data.value_chars).toBe(17);
    expect(sends[0]).toContain('Runtime.evaluate');
    expect(sends[0]).toContain('querySelector');
  });

  it('selects native select options through scoped runtime evaluation', async () => {
    mockFetch([{ id: '1', type: 'page', webSocketDebuggerUrl: 'ws://cdp/1' }]);
    const sends = mockWebSocket({ result: { value: { ok: true, selected: 'remote' } } });
    const data = (await selectBrowserTabOption(controlWorkspace(), '1', 'select[name="location"]', 'remote')).data as any;
    expect(data.selector).toBe('select[name="location"]');
    expect(data.value).toBe('remote');
    expect(sends[0]).toContain('Runtime.evaluate');
    expect(sends[0]).toContain('option_not_found');
  });

  it('submits forms through scoped runtime evaluation', async () => {
    mockFetch([{ id: '1', type: 'page', webSocketDebuggerUrl: 'ws://cdp/1' }]);
    const sends = mockWebSocket({ result: { value: { ok: true, tag: 'form' } } });
    const data = (await submitBrowserTabForm(controlWorkspace(), '1', 'form#draft')).data as any;
    expect(data.selector).toBe('form#draft');
    expect(data.element).toBe('form');
    expect(sends[0]).toContain('Runtime.evaluate');
    expect(sends[0]).toContain('requestSubmit');
  });
  ...
  it('requires browser control for tab mutations', async () => {
    ...
    await expect(fillBrowserTabField(fixtureWorkspace(), '1', 'input', 'x')).rejects.toThrow('browser control is not enabled');
    await expect(selectBrowserTabOption(fixtureWorkspace(), '1', 'select', 'x')).rejects.toThrow('browser control is not enabled');
